...
object IndexedSeqView {
  ...
  @SerialVersionUID(3L)
  case object Empty extends AbstractSeqView[Nothing] with IndexedSeqView[Nothing] {
  ...
  @SerialVersionUID(3L)
  private[scala] final class Single[+A](underlying: A) extends AbstractSeqView[A] with IndexedSeqView[A] {
    override def apply(i: Int) = if (i == 0) underlying else throw new IndexOutOfBoundsException(i.toString)
    override def length = 1
    override def prepended[B >: A](elem: B) = new PreviouslyEvaluated(IndexedSeq(elem, underlying))
    ...
    override def appended[B >: A](elem: B) = new PreviouslyEvaluated(IndexedSeq(underlying, elem))
  }
  ...
  /** In cases where a user constructs previously-evaluated elements, such as in the case of appending/prepending, etc,
    ...
    * storing repeated prepends/appends in an IndexedSeq rather than in the call stack.
    *
    * All other operations are still completely lazy though, in keeping with the idea of a view.
    */
  @SerialVersionUID(3L)
  private[scala] final class PreviouslyEvaluated[+A](underlying: IndexedSeq[A]) extends AbstractSeqView[A] with IndexedSeqView[A] {
